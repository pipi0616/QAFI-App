...
import {
  EvidenceTab, ClinVarTab, AlphaMissenseTab, GnomadTab, LiteratureTab, PositionTab,
  type LookupResult,
...
export default function AgentPage() {
  ...
  const [supportingTab, setSupportingTab] = useState<"evidence" | "clinvar" | "alphamissense" | "gnomad" | "literature" | "position">("evidence");
  ...
  const runAssess = async () => {
    ...
    setLoading(true); setReport(""); setToolCalls([]); setError(""); setMeta(null); setCopied(false); setLookupData(null);
    setSupportingTab("evidence");
    try {
      // Fire both API calls in parallel: AI assessment + structured lookup data
      const [assessRes, lookupRes] = await Promise.allSettled([
        api.assess({ protein_id: protein, gene, variant: variant.trim() }),
        api.lookupVariant(protein, variant.trim()),
      ]);

      if (assessRes.status === "fulfilled") {
        const res = assessRes.value;
        setReport(res.report);
        setToolCalls(res.tool_calls || []);
        setMeta({ variant: res.variant, gene: res.gene, position: res.position, wt: res.wt, mut: res.mut });
      } else {
        setError(assessRes.reason?.message || "Assessment failed");
      }
        ...
        <>
          ...
          {report && (
            <>
              ...
              {lookupData && (
                <>
                  <div style={{ fontSize: 13, fontWeight: 600, color: "#94a3b8", marginTop: 24, marginBottom: 8, textTransform: "uppercase", letterSpacing: 1 }}>
                    Supporting Data
                  </div>
                  <div style={{ display: "flex", background: "#fff", borderRadius: "12px 12px 0 0", border: "1px solid #e2e8f0", borderBottom: "none", padding: "0 8px", overflowX: "auto" }}>
                    {[
                      ...
                      <button key={t.key} onClick={() => setSupportingTab(t.key as any)} style={{
                        padding: "10px 14px", fontSize: 13, fontWeight: supportingTab === t.key ? 600 : 400,
                        color: supportingTab === t.key ? "#0f172a" : "#94a3b8", background: "transparent",
                        border: "none", borderBottom: supportingTab === t.key ? "2px solid #3b82f6" : "2px solid transparent",
                        cursor: "pointer", display: "flex", alignItems: "center", gap: 5, whiteSpace: "nowrap",
                      }}>
                        {t.label}
                        {t.key === "clinvar" && lookupData.clinvar &&
                          <span style={{ width: 7, height: 7, borderRadius: "50%", background: lookupData.clinvar.found ? "#10b981" : "#cbd5e1" }} />}
                        {t.key === "alphamissense" && lookupData.alphamissense?.variant &&
                          <span style={{ width: 7, height: 7, borderRadius: "50%", background: lookupData.alphamissense.variant.am_class_color }} />}
                        {t.key === "gnomad" && lookupData.gnomad?.variant &&
                          <span style={{ width: 7, height: 7, borderRadius: "50%", background: lookupData.gnomad.variant.freq_color }} />}
                        {t.key === "literature" && lookupData.literature &&
                          <span style={{ fontSize: 11, color: "#64748b", fontWeight: 400 }}>
                            {(lookupData.literature.variant_search_count || 0) + (lookupData.literature.gene_search_count || 0)}
                          </span>}
                      </button>
                    ))}
                  </div>
                  <div style={{ ...card, borderRadius: "0 0 12px 12px", minHeight: 200 }}>
                    {supportingTab === "evidence" && <EvidenceTab result={lookupData} />}
                    {supportingTab === "clinvar" && <ClinVarTab result={lookupData} />}
                    {supportingTab === "alphamissense" && <AlphaMissenseTab result={lookupData} />}
                    {supportingTab === "gnomad" && <GnomadTab result={lookupData} />}
                    {supportingTab === "literature" && <LiteratureTab result={lookupData} />}
                    {supportingTab === "position" && <PositionTab result={lookupData} />}
                  </div>
                </>
              )}
            </>
